...
const STYLES = `
  ...
  *{box-sizing:border-box}
  ...
  .btn{width:100%;padding:14px;background:linear-gradient(135deg,#10b981,#059669);color:white;border:none;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;font-family:'Inter',sans-serif;transition:all 0.2s}
...
// Sub-components
// ─────────────────────────────────────────────
function SuccessView({ isReset, msg, onBack }: {
  isReset: boolean;
  msg: string;
  onBack: () => void;
}) {
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      {!isReset && (
        <button onClick={onBack} className="btn" style={{ marginTop: 24 }}>
          Back to Sign In
        </button>
      )}
    </div>
...
function ErrorMessage({ error }: { error: string }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 10, padding: '11px 14px', fontSize: 13, color: '#f87171',
    }}>
      ⚠️ {error}
    </div>
  );
...
function RequestResetForm({ email, setEmail, onSubmit }: {
  email: string;
...
function UpdatePasswordForm({ password, setPassword, confirm, setConfirm, onSubmit }: {
  password: string;
  ...
  return (
    <>
      <div>
        <div style={LABEL_STYLE}>New Password</div>
        <input
          value={password}
...
function ResetPasswordForm() {
  ...
  const handleSubmit = isReset ? updatePassword : requestReset;
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{
          ...
          {done ? (
            <SuccessView isReset={isReset} msg={msg} onBack={() => router.push('/login')} />
          ) : (
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {isReset ? (
                  ...
                  <RequestResetForm email={email} setEmail={setEmail} onSubmit={handleSubmit} />
                )}
                ...
                <button
                  ...
                  style={{
                    ...
                    cursor: 'pointer', fontFamily: "'Inter',sans-serif", padding: 8,
                  }}
                >
                  ← Back to Sign In
                </button>
