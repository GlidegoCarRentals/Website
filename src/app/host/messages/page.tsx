...
export default function HostMessagesPage() {
  ...
  const sendMessage = (msg?: string) => {
    ...
    if (!content) return;
    const newMsg = { id: Date.now(), from: 'host' as const, text: content, time: new Date().toLocaleTimeString('en-AU', { hour: '2-digit', minute: '2-digit' }), date: 'Today' };
    setConversations(cs => cs.map(c => c.id === selected.id ? { ...c, messages: [...c.messages, newMsg], lastMsg: content, lastTime: 'now', unread: 0 } : c));
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ width: 300, borderRight: '1px solid #f1f5f9', display: 'flex', flexDirection: 'column', background: 'white', flexShrink: 0 }}>
          <div style={{ padding: '14px 16px', borderBottom: '1px solid #f1f5f9' }}>
            <input style={{ width: '100%', border: '1.5px solid #f1f5f9', borderRadius: 10, padding: '9px 14px', fontSize: 13, outline: 'none', background: '#f8fafc', color: '#0f172a' }}
              placeholder="🔍  Search conversations..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
            {filtered.map(c => (
              <div key={c.id} className={`conv-item ${selected.id === c.id ? 'active' : ''}`} onClick={() => { setSelected(c); setConversations(cs => cs.map(x => x.id === c.id ? { ...x, unread: 0 } : x)); }}>
                <div style={{ width: 44, height: 44, borderRadius: '50%', background: `linear-gradient(135deg,${c.unread ? '#1d4ed8' : '#64748b'},${c.unread ? '#059669' : '#94a3b8'})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: 'white', flexShrink: 0, position: 'relative' }}>
                  {c.avatar}
                  {c.unread > 0 && <div style={{ position: 'absolute', top: -2, right: -2, width: 16, height: 16, background: '#ef4444', borderRadius: '50%', fontSize: 9, fontWeight: 800, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid white' }}>{c.unread}</div>}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: '#0f172a' }}>{c.guest}</span>
                    <span style={{ fontSize: 11, color: '#94a3b8' }}>{c.lastTime}</span>
                  </div>
                  <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 2 }}>🚗 {c.car}</div>
                  <div style={{ fontSize: 12, color: '#64748b', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.lastMsg}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            {selected.messages.map((msg, i) => {
              ...
              return (
                <div key={msg.id}>
                  {showDate && (
                    <div style={{ textAlign: 'center', fontSize: 11, color: '#94a3b8', fontWeight: 600, margin: '8px 0', letterSpacing: '0.05em' }}>{msg.date}</div>
                  )}
                  <div style={{ display: 'flex', justifyContent: msg.from === 'host' ? 'flex-end' : 'flex-start', alignItems: 'flex-end', gap: 8 }}>
                    {msg.from === 'guest' && (
                      <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg,#64748b,#94a3b8)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: 'white', flexShrink: 0 }}>{selected.avatar}</div>
                    )}
                    <div>
                      <div className={`msg-bubble ${msg.from === 'host' ? 'msg-host' : 'msg-guest'}`}>{msg.text}</div>
                      <div style={{ fontSize: 10, color: '#94a3b8', marginTop: 4, textAlign: msg.from === 'host' ? 'right' : 'left' }}>{msg.time}</div>
                    </div>
                  </div>
                </div>
              );
          ...
          {/* Quick replies */}
          <div style={{ padding: '12px 24px 0', background: 'white', borderTop: '1px solid #f1f5f9', display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 10 }}>
            {QUICK_REPLIES.map((r, i) => (
              <button key={i} className="quick-btn" onClick={() => sendMessage(r)}>{r.slice(0, 30)}{r.length > 30 ? '…' : ''}</button>
            ))}
          </div>

          {/* Input */}
          <div style={{ padding: '12px 24px 16px', background: 'white', display: 'flex', gap: 10, alignItems: 'flex-end' }}>
            <textarea className="inp" value={text} onChange={e => setText(e.target.value)} placeholder={`Message ${selected.guest}...`} rows={1}
              onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } }}
              style={{ lineHeight: 1.5 }} />
            <button className="send-btn" onClick={() => sendMessage()} title="Send">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5"><path d="m22 2-7 20-4-9-9-4 20-7z" /><path d="m22 2-11 11" /></svg>
            </button>
          </div>
        </div>
      </div>
