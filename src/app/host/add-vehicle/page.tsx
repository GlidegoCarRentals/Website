...
// ─────────────────────────────────────────────
// Singleton client — module level, not per-render
// ─────────────────────────────────────────────
...
const FUEL_ICONS: Record<string, string> = { Petrol: '⛽', Diesel: '🛢️', Hybrid: '🌿', Electric: '⚡', 'Plug-in Hybrid (PHEV)': '🔌' }
...
// TODO — replace with real PPSR API call to /api/rego/lookup
// ─────────────────────────────────────────────
...
function Label({ htmlFor, children, required }: Readonly<{ htmlFor?: string; children: React.ReactNode; required?: boolean }>) {
  return (
    <label htmlFor={htmlFor} className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">
...
function selectBorder(ok: boolean, err: boolean) {
  if (ok)  return '1px solid rgba(16,185,129,0.4)'
  if (err) return '1px solid rgba(239,68,68,0.5)'
...
type SetTouched = React.Dispatch<React.SetStateAction<Record<string, boolean>>>

interface BaseStepProps {
  form: Form
  set: SetField
  setTouched: SetTouched
  errMsg: (key: string) => string | undefined
  isOk: (key: string) => boolean
...
// ─────────────────────────────────────────────
interface Step0Props extends BaseStepProps {
  regoLoad: boolean
  regoOk: boolean
  regoWarnings: string[]
  handleRegoLookup: () => void
}

function VehicleDetailsStep({ form, set, setTouched, errMsg, isOk, isErr, regoLoad, regoOk, regoWarnings, handleRegoLookup }: Readonly<Step0Props>) {
  return (
    <div>
      {/* REGO auto-fill */}
      <div style={{ background: 'linear-gradient(135deg, rgba(37,99,235,0.08), rgba(139,92,246,0.05))', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 18, padding: 20, marginBottom: 28 }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
          <div style={{ flex: 1, position: 'relative' }}>
            <label htmlFor="add-rego" className="sr-only">Registration number</label>
            <input
              id="add-rego"
              type="text"
              value={form.rego}
              onChange={e => { set('rego', e.target.value.toUpperCase().replaceAll(/[^A-Z0-9]/g, '')); }}
          ...
          <label htmlFor="add-rego-state" className="sr-only">State</label>
          <select id="add-rego-state" value={form.rego_state} onChange={e => set('rego_state', e.target.value)}
        ...
        <p style={{ color: '#334155', fontSize: 10, marginTop: 8, textAlign: 'center' }}>Demo mode — real PPSR API ready to connect when needed</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginBottom: 18 }}>
        <div>
          <Label htmlFor="add-make" required>Make</Label>
          <div style={{ position: 'relative' }}>
            <input id="add-make" type="text" value={form.make}
        ...
        <div>
          <Label htmlFor="add-model" required>Model</Label>
          <div style={{ position: 'relative' }}>
            <input id="add-model" type="text" value={form.model}
        ...
        <div>
          <Label htmlFor="add-colour" required>Colour</Label>
          <div style={{ position: 'relative' }}>
            <input id="add-colour" type="text" value={form.colour}
              onChange={e => set('colour', e.target.value.replaceAll(/[^a-zA-Z ]/g, ''))}
        ...
        <div>
          <Label htmlFor="add-engine" required>Engine</Label>
          <div style={{ position: 'relative' }}>
            <input id="add-engine" type="text" value={form.engine}
        ...
        <div>
          <Label htmlFor="add-year" required>Year</Label>
          <div style={{ position: 'relative' }}>
            <input id="add-year" type="number" value={form.year}
        ...
        <div>
          <Label htmlFor="add-seats" required>Seats</Label>
          <select id="add-seats" value={form.seats} onChange={e => set('seats', Number.parseInt(e.target.value))}
        ...
        <div>
          <Label htmlFor="add-body-type" required>Body Type</Label>
          <select id="add-body-type" value={form.body_type} onChange={e => set('body_type', e.target.value)}
            style={{ width: '100%', padding: '12px 16px', background: 'rgba(30,40,60,0.8)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, color: 'white', fontSize: 14, outline: 'none' }}>
        ...
        <div>
          <Label htmlFor="add-transmission" required>Transmission</Label>
          <select id="add-transmission" value={form.transmission} onChange={e => set('transmission', e.target.value)}
            style={{ width: '100%', padding: '12px 16px', background: 'rgba(30,40,60,0.8)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, color: 'white', fontSize: 14, outline: 'none' }}>
      ...
      {/* Pickup Location */}
      <div>
        <Label htmlFor="add-location" required>Pickup Location</Label>
        <div style={{ position: 'relative' }}>
          <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', fontSize: 15, pointerEvents: 'none' }}>📍</span>
          <select id="add-location" value={form.location}
            onChange={e => { set('location', e.target.value); setTouched(p => ({ ...p, location: true })) }}
            style={{ width: '100%', padding: '12px 16px 12px 40px', background: isOk('location') ? 'rgba(16,185,129,0.05)' : 'rgba(30,40,60,0.8)', border: selectBorder(isOk('location'), isErr('location')), borderRadius: 12, color: 'white', fontSize: 14, outline: 'none' }}>
...
function PhotosStep({ form, set, uploading, uploadPct, drag, setDrag, fileRef, dragRef, handleFileInput, handleDrop, errors, touched }: Readonly<Step1Props>) {
  return (
    <div>
      ...
      <div
        ...
        role="button"
        tabIndex={0}
        aria-label="Upload vehicle photos — drag and drop or click to browse"
      ...
      <input ref={fileRef} id="photo-upload" type="file" accept="image/*" multiple onChange={handleFileInput} className="hidden" aria-label="Choose photos to upload" />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
          {form.photos.map((url, i) => (
            <div key={url} style={{ position: 'relative', borderRadius: 14, overflow: 'hidden', aspectRatio: '16/10', background: '#0d1117', border: i === 0 ? '2px solid #3b82f6' : '1px solid rgba(255,255,255,0.06)' }}
              ...
              <button className="del-btn" aria-label={`Remove photo ${i + 1}`} onClick={() => set('photos', form.photos.filter((_, j) => j !== i))}
...
// Step 2 — Features
// ─────────────────────────────────────────────
function FeaturesStep({ form, set, setTouched, errMsg, isErr }: Readonly<Pick<BaseStepProps, 'form' | 'set' | 'setTouched' | 'errMsg' | 'isErr'>>) {
  return (
    <div>
      ...
      <div style={{ marginTop: 8 }}>
        <Label htmlFor="add-description">
          Description <span style={{ color: '#475569', fontWeight: 400, textTransform: 'none', fontSize: 11 }}>(optional, 30+ characters)</span>
        </Label>
        <textarea id="add-description" value={form.description}
...
// Step 3 — Pricing
// ─────────────────────────────────────────────
function PricingStep({ form, set, setTouched, errMsg, isOk, isErr }: Readonly<BaseStepProps>) {
  const rateFields = [
    { label: 'Weekly Rate',  hint: '~6× daily',  field: 'price_weekly'  as const },
    { label: 'Monthly Rate', hint: '~22× daily', field: 'price_monthly' as const },
  ]

  const toggleFields = [
    { field: 'instant_book'        as const, label: 'Instant Book',       desc: 'Guests book without approval needed',           icon: '⚡' },
    { field: 'delivery_available'  as const, label: 'Delivery Available', desc: 'You deliver the car to airports, hotels, etc.',  icon: '🚚' },
  ]
  ...
  return (
    <div>
      ...
      <div style={{ background: 'linear-gradient(135deg, rgba(16,185,129,0.08), rgba(5,150,105,0.05))', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 18, padding: 20, marginBottom: 24 }}>
        <Label htmlFor="add-price-daily" required>Daily Rate</Label>
        <div style={{ position: 'relative', marginBottom: 4 }}>
          ...
          <input id="add-price-daily" type="number" value={form.price_daily || ''}
            onChange={e => {
              const v = Number.parseFloat(e.target.value) || 0
              set('price_daily', v)
              if (!form.price_weekly)  set('price_weekly',  Math.round(v * 6))
              if (!form.price_monthly) set('price_monthly', Math.round(v * 22))
            }}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
        {rateFields.map(({ label, hint, field }) => (
          <div key={field}>
            <Label htmlFor={`add-${field}`}>{label} <span style={{ color: '#334155', fontWeight: 400, textTransform: 'none', fontSize: 10 }}>({hint})</span></Label>
            <div style={{ position: 'relative' }}>
              <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#475569', fontWeight: 600, fontSize: 12 }}>AUD $</span>
              <input id={`add-${field}`} type="number" value={form[field] || ''}
                onChange={e => set(field, Number.parseFloat(e.target.value) || 0)}
                onBlur={() => setTouched(p => ({ ...p, [field]: true }))}
                placeholder={String(field === 'price_weekly' ? Math.round(form.price_daily * 6) : Math.round(form.price_daily * 22))}
                className={inputCls(errMsg(field), isOk(field))}
                style={{ paddingLeft: 58, paddingRight: 32 }} />
              <FieldOk show={isOk(field)} />
            </div>
            <FieldError msg={errMsg(field)} />
          </div>
        ))}

        <div>
          <Label htmlFor="add-deposit" required>Security Bond</Label>
          <div style={{ position: 'relative' }}>
            <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#475569', fontWeight: 600, fontSize: 12 }}>AUD $</span>
            <input id="add-deposit" type="number" value={form.deposit_amount || ''}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
        <div>
          <Label htmlFor="add-min-days" required>Minimum Days</Label>
          <select id="add-min-days" value={form.min_days} onChange={e => set('min_days', Number.parseInt(e.target.value))}
            style={{ width: '100%', padding: '12px 16px', background: 'rgba(30,40,60,0.8)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, color: 'white', fontSize: 14, outline: 'none' }}>
            {MIN_DAYS_OPTS.map(n => <option key={n} value={n}>{n} day{n > 1 ? 's' : ''}</option>)}
          </select>
        </div>
        <div>
          <Label htmlFor="add-min-age" required>Minimum Driver Age</Label>
          <select id="add-min-age" value={form.min_age_years} onChange={e => set('min_age_years', Number.parseInt(e.target.value))}
            style={{ width: '100%', padding: '12px 16px', background: 'rgba(30,40,60,0.8)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, color: 'white', fontSize: 14, outline: 'none' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {toggleFields.map(({ field, label, desc, icon }) => {
          const active = form[field]
          return (
            <button key={field} onClick={() => set(field, !active)}
        ...
        {form.delivery_available && (
          <div style={{ marginTop: 4 }}>
            <Label htmlFor="add-delivery-fee" required>Delivery Fee</Label>
            <div style={{ position: 'relative' }}>
              <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#475569', fontWeight: 600, fontSize: 12 }}>AUD $</span>
              <input id="add-delivery-fee" type="number" value={form.delivery_fee || ''}
...
// ─────────────────────────────────────────────
function ReviewStep({ form }: Readonly<{ form: Form }>) {
...
export default function AddVehiclePage() {
  ...
  useEffect(() => {
    if (loading) return
    if (!user) { router.push('/login?redirectTo=/host/add-vehicle'); return }
  ...
  const uploadFiles = async (files: File[]) => {
    ...
    for (let i = 0; i < batch.length; i++) {
      ...
      if (!file.type.startsWith('image/')) { setGlobalErr(`${file.name} is not an image file.`); continue }
      if (file.size > 10 * 1024 * 1024)   { setGlobalErr(`${file.name} exceeds 10 MB limit.`);  continue }
      ...
      if (upErr) { urls.push(URL.createObjectURL(file)); console.warn('Upload error (local preview):', upErr.message); continue }
  ...
  const goNext = () => {
    ...
    if (Object.keys(errs).length > 0) {
      ...
      setGlobalErr(Object.values(errs)[0] ?? 'Please fix the errors above before continuing.')
      return
    }
    setGlobalErr('')
    setStep(s => s + 1)
  ...
  const handleSubmit = async () => {
    ...
    const { error: err } = await supabase.from('cars').insert({
      ...
      slug:               `${form.make}-${form.model}-${form.year}-${Date.now()}`.toLowerCase().replaceAll(/[^a-z0-9]+/g, '-'),
    })
    setSubmitting(false)
  ...
  const baseStepProps: BaseStepProps = { form, set, setTouched, errMsg, isOk, isErr }
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #06090f 0%, #0a1020 50%, #06090f 100%)', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '36px 20px 80px' }}>
        ...
        <div style={{ background: 'rgba(13,17,26,0.9)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 24, padding: 32, boxShadow: '0 32px 80px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.03)', backdropFilter: 'blur(20px)' }}>
          ...
          {step === 0 && <VehicleDetailsStep {...baseStepProps} regoLoad={regoLoad} regoOk={regoOk} regoWarnings={regoWarnings} handleRegoLookup={handleRegoLookup} />}
          {step === 1 && <PhotosStep form={form} set={set} uploading={uploading} uploadPct={uploadPct} drag={drag} setDrag={setDrag} fileRef={fileRef} dragRef={dragRef} handleFileInput={handleFileInput} handleDrop={handleDrop} errors={errors} touched={touched} />}
          {step === 2 && <FeaturesStep form={form} set={set} setTouched={setTouched} errMsg={errMsg} isErr={isErr} />}
          {step === 3 && <PricingStep {...baseStepProps} />}
