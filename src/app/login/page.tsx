...
function LoginForm() {
  ...
  const visibleError =
    error ||
    (queryError ? decodeURIComponent(queryError) : '') ||
  ...
  useEffect(() => {
    ...
    if (hashError || hash.includes('access_token')) {
      window.history.replaceState(null, '', window.location.pathname);
    }
  }, [hash, hashError, router]);
    ...
    <div style={{ minHeight: '100vh', background: '#070d1a', display: 'flex', fontFamily: "'Inter',sans-serif" }}>
      ...
      <div className="left-panel" style={{ flex:1, background:'linear-gradient(135deg,#060d1e,#0a1628,#071a10)', display:'flex', flexDirection:'column', justifyContent:'space-between', padding:'48px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          {[
            { icon:'Lightning', label:'Instant booking with no waiting', color:'#10b981' },
            { icon:'Shield', label:'Full insurance included on every trip', color:'#3b82f6' },
            { icon:'Star', label:'4.9 star average from 3,000+ customers', color:'#f59e0b' },
          ].map((feature) => (
            <div key={feature.label} style={{ display:'flex', alignItems:'center', gap:12 }}>
              <div style={{ width:36, height:36, borderRadius:10, background:feature.color+'20', border:`1px solid ${feature.color}33`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:800, color:feature.color }}>
                {feature.icon[0]}
              </div>
              <span style={{ fontSize:13, color:'rgba(255,255,255,0.65)' }}>{feature.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ width:'100%', maxWidth:480, display:'flex', flexDirection:'column', justifyContent:'center', padding:'48px 40px', overflowY:'auto' }}>
        <div style={{ display:'flex', background:'rgba(255,255,255,0.04)', borderRadius:14, padding:4, marginBottom:32 }}>
          {(['login','signup'] as const).map((currentMode) => (
            <button
              key={currentMode}
              onClick={() => { setMode(currentMode); setError(''); setSuccess(''); }}
              style={{ flex:1, padding:'11px', borderRadius:10, border:'none', cursor:'pointer', fontSize:14, fontWeight:700, fontFamily:"'Inter',sans-serif", background: mode===currentMode ? 'linear-gradient(135deg,#10b981,#059669)' : 'transparent', color: mode===currentMode ? 'white' : 'rgba(255,255,255,0.4)', transition:'all 0.2s' }}
            >
        ...
        {mode === 'signup' && (
          <div style={{ display:'flex', gap:10, marginBottom:20 }}>
            {([
              { val:'guest', icon:'R', label:'Rent a Car', sub:'I want to book' },
              { val:'host', icon:'H', label:'List My Car', sub:'I want to earn' },
            ] as const).map((option) => (
              <button
                key={option.val}
                onClick={() => setRole(option.val)}
                style={{ flex:1, padding:'14px 12px', borderRadius:14, border:`2px solid ${role===option.val ? '#10b981' : 'rgba(255,255,255,0.08)'}`, background: role===option.val ? 'rgba(16,185,129,0.08)' : 'rgba(255,255,255,0.03)', cursor:'pointer', textAlign:'center', transition:'all 0.2s' }}
              >
                <div style={{ fontSize:24, marginBottom:4 }}>{option.icon}</div>
                <div style={{ fontSize:12, fontWeight:700, color: role===option.val ? '#10b981' : 'rgba(255,255,255,0.7)' }}>{option.label}</div>
                <div style={{ fontSize:10, color:'rgba(255,255,255,0.3)', marginTop:2 }}>{option.sub}</div>
              </button>
            ))}
          </div>
        )}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:14, marginBottom:20 }}>
          ...
          <div>
            ...
            <div style={{ position:'relative' }}>
              <input value={password} onChange={(e) => setPassword(e.target.value)} type={showPass ? 'text' : 'password'} placeholder="Password" className="inp" onKeyDown={(e) => e.key === 'Enter' && handleSubmit()} />
              <button type="button" onClick={() => setShowPass(!showPass)} style={{ position:'absolute', right:14, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:'rgba(255,255,255,0.3)', fontSize:16 }}>
                {showPass ? 'Hide' : 'Show'}
              </button>
            </div>
          </div>
        ...
        {mode === 'login' && (
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:4 }}>
            <div onClick={() => setRememberMe(!rememberMe)} style={{ width:20, height:20, borderRadius:6, border:`2px solid ${rememberMe ? '#10b981' : 'rgba(255,255,255,0.2)'}`, background: rememberMe ? '#10b981' : 'transparent', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, transition:'all 0.2s' }}>
              {rememberMe && <span style={{ color:'white', fontSize:12, fontWeight:700 }}>OK</span>}
            </div>
        ...
        <p style={{ textAlign:'center', fontSize:12, color:'rgba(255,255,255,0.25)' }}>
          ...
          <button onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setError(''); setSuccess(''); }} style={{ background:'none', border:'none', color:'#10b981', fontWeight:600, cursor:'pointer', fontSize:12, fontFamily:"'Inter',sans-serif" }}>
